...
export default async function handler(req, res) {
  ...
  try {
    ...
    const user = await User.findById(userId).select('followers').lean(); // 使用 Mongoose 查找用户并只选择 followers 字段
    ...
    const validFollowerObjectIds = followerUserIds // Mongoose 返回的 ObjectId 已经是 ObjectId 类型
    ...
    const followerUsers = await User.find( // 使用 Mongoose 查找多个用户
